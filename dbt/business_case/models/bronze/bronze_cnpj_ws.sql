-- Bronze layer: CNPJ data from temp layer
...
WITH temp_data AS (
    ...
    FROM `{{ var('gcp_project') }}.business_case_temp.cnpj_ws`
),

existing_bronze AS (
    SELECT *
    FROM `{{ var('gcp_project') }}.business_case_bronze.cnpj_ws`
...
merged_data AS (
    ...
    WHERE NOT EXISTS (
        ...
        WHERE existing_bronze.zipcode = temp_data.zipcode -- Use appropriate key
    )
)
